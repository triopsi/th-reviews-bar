...
/* Hooks the metabox */
add_action('admin_init', 'thrb_add_partner', 1);

function thrb_add_partner() {
	add_meta_box( 
		'thrb-partner-url', 
...
function thrb_add_review_details( $post ){
	...
	<div class="thrb_field">
		<div class="thrb_field_title">
			<?php echo __('Author name','thrb'); ?>
		</div>
		<input class="thrb-field regular-text" id="reviewDetailsAuthorNameField" name="thrb_review_details_author_name" type="text" value="<?php echo esc_html( $reviewauthor ) ?>" placeholder="<?php echo __('e.g. Max','thrb'); ?>">
		<br>
		<div class="thrb_field_title">
			<?php echo __('Rating','thrb'); ?>
		</div>
		<input class="thrb-field regular-text" id="reviewDetailsRatingField" name="thrb_review_details_rating" type="number" min="0" max="5" step="0.5" value="<?php echo esc_html( $reviewrating ) ?>" >
		<br>
		<hr>
